module.exports = function () {
    ...
    function getComments(res, mysql, context, complete) {
        mysql.pool.query("SELECT * FROM comments INNER JOIN users ON users.user_id = comments.user_id", function (error, results, fields) {
            if (error) {
    ...
    function getCommentsByPost(req, res, mysql, context, complete) {
        var query = "SELECT c.comment_id, c.comment_date, u.username, p.post_id, c.content FROM comments c INNER JOIN posts p ON c.post_id = p.post_id INNER JOIN users u ON u.user_id = c.user_id WHERE p.post_id = ?";
